function [PowSpectrum,NorFreq,SegmentNum] = Classic_ImpPeriodogramAver( SignalIn,SignalLength,SampleNumPerSegment,OverlapNum,WindowType,UnitType )
%FileName: Classic_ImpPeriodogramAver.m
%Description: 用平均法改进的周期图法
if size(SignalIn,2)>1   %输入信号列数大于1报错
    error(message('Error! Invaild Input Signal Demension @ Function:"Classic_ImpPeriodogramAver.m"'));
end
if SignalLength<=0||SampleNumPerSegment<=0||OverlapNum<0
    error(message('Error! Window Size or Sample Number per Segment or Overlap Number @ Function:"Classic_ImpPeriodogramAver.m"'));
end
InputSignal=SignalIn(1:SignalLength,1);
if OverlapNum==0
   if mod(SignalLength,SampleNumPerSegment)~=0
       error(message('Error! WindowSize and SampleNumPerSegment are not match to each other @ Function:"Classic_ImpPeriodogramAver.m"'));
   end
...
WindowCoeff=Assist_WindowSelect(WindowType,SampleNumPerSegment);
Index=1;
while Index<=SegmentNum
    SegmentPowerSpectrum(:,Index)=Classic_Periodogram(SegmentSignal(:,Index).*WindowCoeff,SampleNumPerSegment,0);
    Index=Index+1;
end
