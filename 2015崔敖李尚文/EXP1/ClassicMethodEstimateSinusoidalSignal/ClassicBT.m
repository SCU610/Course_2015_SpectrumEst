function [PowSpectrum,NorFreq] = ClassicBT(SignalIn,SignalLength,UnitType)
%FileName: ClassicBT.m
%Description: 使用经典BT法计算输入信号的功率谱
%   函数输入: 输入信号，开窗大小
%   函数输出: 计算好的功率谱和归一化频率
if size(SignalIn,2)>1   %输入信号列数大于1报错
    error(message('Error! Invaild Input Signal Demension @ Function:"ClassicBT.m"'));
end
